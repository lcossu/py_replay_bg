...
class ReplayBGData:
    ...
    hr: np.ndarray
        An array containing the heart rate data (beats/min).
    ...
    def __init__(self,
        ...
        cho_source : str, {'data', 'generated'}, optional, default : 'data'
            A string defining whether to use, during replay, the CHO data contained in the 'data' timetable (if 'data'),
            or the CHO generated by the meal generator implemented via the provided 'mealGeneratorHandler' function.
        hr: pd.DataFrame, optional, default : None
            A pandas dataframe which contains the heart rate data to be used by the tool. It is important that the
            dataframe start and end time are the same of data and the sampling time is either 1 or 5 minutes.
        ...
        self.hr = []
        self.hr = np.full([model.tsteps, ], np.nan)
        self.hr_idxs = []
        if hr is not None:
            dt = (hr.t[1] - hr.t[0]).total_seconds() / 60  # in minutes
            if dt == environment.yts: # sampling of 5 minutes, expand to 1 minute
                hr_raw = hr.hr.values.astype(float)
                for t in range(data.shape[0]):
                    start = t * environment.yts
                    end = (t + 1) * environment.yts
                    self.hr[start:end] = hr_raw[t]
                self.hr_idxs = np.where(~np.isnan(self.hr))[0]
            else:
                self.hr = hr.hr.values.astype(float)
                self.hr_idxs = np.where(~np.isnan(self.hr))[0]
    ...
    def __insulin_setup(self,
        ...
        if self.bolus_source == 'data':
            ...
            for i in range(np.size(b_idx)):
                self.bolus[(b_idx[i] * environment.yts): ((b_idx[i] + 1) * environment.yts)] = data['bolus'][b_idx[
                    i]] * model.model_parameters.to_mgkg  # mU/(kg*min)
                self.bolus_label[(b_idx[i] * environment.yts): ((b_idx[i] + 1) * environment.yts)] = \
    ...
    def __meal_setup(self,
        ...
        if self.cho_source == 'data':
            ...
            for i in range(np.size(m_idx)):
                self.meal[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = data['cho'][m_idx[
                    i]] * model.model_parameters.to_mgkg  # mg/(kg*min)
                self.meal_announcement[(m_idx[i] * environment.yts)] = data['cho'][
                                                                           m_idx[i]] * environment.yts  # mg/(kg*min)
                ...
                if environment.blueprint == 'single-meal':
                    ...
                    if i == 0:
                        ...
                        self.meal_M[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = self.meal[
                            (m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)]
                    else:
                        self.meal_type[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = 'O'
                        self.meal_O[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = self.meal[
                            (m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)]

                if environment.blueprint == 'multi-meal':
                    self.meal_type[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = data['cho_label'][
                        m_idx[i]]

                    if data['cho_label'][m_idx[i]] == 'B':
                        self.meal_B[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = self.meal[
                            (m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)]
                    if data['cho_label'][m_idx[i]] == 'L':
                        self.meal_L[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = self.meal[
                            (m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)]
                    if data['cho_label'][m_idx[i]] == 'D':
                        self.meal_D[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = self.meal[
                            (m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)]
                    if data['cho_label'][m_idx[i]] == 'S':
                        self.meal_S[(m_idx[i] * environment.yts):(
                                (m_idx[i] + 1) * environment.yts)] = self.meal[
                            (m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)]
                    if data['cho_label'][m_idx[i]] == 'H':
                        self.meal_H[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = self.meal[
                            (m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)]

                    if data['cho_label'][m_idx[i]] == 'B2':
                        self.meal_B2[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = self.meal[
                            (m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)]
                    if data['cho_label'][m_idx[i]] == 'L2':
                        self.meal_L2[(m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)] = self.meal[
                            (m_idx[i] * environment.yts):((m_idx[i] + 1) * environment.yts)]
